...
if __name__=='__main__':
    ...
    stats_list, summary = computeStats(dirData)


    print 'Number of subDirs: ', summary.loc['count'][0]
    print 'Total of items: ', sum(stats_list['count'])
    print '(Round) Median of file by subdirectory: ', int(round(summary.loc['mean'][0]))
    print '(Round) Standart desviation: ', int(round(summary.loc['std'][0]))
    print 'Minimum files in subdir: ', int(round(summary.loc['min'][0]))
    print 'Maximum files in subdir: ', int(round(summary.loc['max'][0]))


    # Plot the next three items: Number of items by folder, mean-std, mean+std
    plot(stats_list, summary.loc['mean'][0], summary.loc['mean'][0]-summary.loc['std'][0], summary.loc['mean'][0]+summary.loc['std'][0])


    ''' THIS WAS FOR MY OWN PROPORSE, YOU DON'T NEED THIS

############################################### Useful folders

    print ' Stats for folders with [ #items >= (mean-std) ] '

    # Plot again, but this time just the folders with [ items >= (mean-std) ]
    constraintData = { k:v for k,v in dirData.items() if len(v) >= summary.loc['mean'][0]-summary.loc['std'][0]}

    stats_list, summary = computeStats(constraintData)
